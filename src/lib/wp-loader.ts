...
export function wpCollections() {
  let endpoint = WP_REST_API_ENDPOINT
	if (!endpoint) {
		throw new AstroError(
			'Missing `WP_REST_API_ENDPOINT` env variable.',
			'Please pass a URL to your WordPress REST API endpoint as the `WP_REST_API_ENDPOINT` ENV variable. Most commonly this looks something like `https://example.com/wp-json/`'
		);
	}
	...
	const l = (type: string) =>
		makeLoader({ name: `wp-${type}`, url: new URL(`wp/v2/${type}`, apiBase) }) as Loader | any;
